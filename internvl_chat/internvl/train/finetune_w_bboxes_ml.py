...
from internvl.model.internvl_chat import (InternVisionConfig,
                                          ...
                                          InternVLChatConfig,
                                          InternVLChatModel,
                                          InternVLChatModelWBbox,
                                          InternVLChatModelWBboxML)
...
class LazySupervisedDataset(Dataset):
    ...
    def video_get_item(self, data_item):
        ...
        video_path = os.path.join(self.root, video_file.split('/')[-1].split('-')[0], video_file)
        bboxes_path = os.path.join(self.root.replace('/data', '/data3').replace('IntentVC_Enhanced_Annotated', 'IntentVC'), video_file.split('/')[-1].split('-')[0], video_file.split('/')[-2], 'object_bboxes.npy')
        bboxes = np.load(bboxes_path)
        # Load the video frames using tcs_loader
        # TODO: Load videos without using tcsloader.

        image_list, frame_indices = self.tcs_loader(
            video_path,
            image_type='video',
        ...
        bboxes_str = " The coordinates in each frame are:\n"
        name = video_file.split('/')[-1].split('-')[0]
        if target_transfer.get(name, None) is not None:
            print(f"{video_path.split('/')[-1]} -> {target_transfer[name]}")
            name = target_transfer[name]
    
        for index, bbox in enumerate(bboxes):
            # <ref>class name</ref><box>[[x1, y1, x2, y2], ...]</box>
            bboxes_str += f'{index+1}. ' + "<ref>" + f'{name}' + "</ref><box>[" + f'{bbox.tolist()}' + "]</box>\n"

        data_item['conversations'][0]['value'] += bboxes_str
        data_item['conversations'][0]['value'] += '.'
        ...
        # if bboxes.shape[0] != num_patches:
        #     print(bboxes.shape[0], num_patches)
        # import torchvision.transforms as T
        # import cv2
        # mean = np.asarray([ 0.485, 0.456, 0.406 ])
        # std = np.asarray([ 0.229, 0.224, 0.225 ])
        # denormalize = T.Normalize((-1 * mean / std), (1.0 / std))
        # for ii, out_image in enumerate(pixel_values):
        #     out_image = denormalize(out_image)
        #     out_image = out_image.permute(1, 2, 0).detach().cpu().numpy()*255.
        #     out_image = out_image.astype(np.uint8)
        #     out_image = cv2.cvtColor(out_image, cv2.COLOR_RGB2BGR)
        #     os.makedirs(f"/data3/vis_out/{video_file.split('/')[-2]}", exist_ok=True)
        ...
        #     cv2.imwrite(f"/data3/vis_out/{video_file.split('/')[-2]}/{ii}.png", out_image)


        # print(num_patches, image_list[0])
        # exit()
        # Select the appropriate preprocessing function based on the template name
        preprocess_function = self.get_preprocess_function()
...
def main():
    ...
    if model_args.model_name_or_path is not None:
        logger.info('Loading InternVLChatModelWBboxML...')
        ...
        model = InternVLChatModelWBboxML.from_pretrained(
        ...
        logger.info('Building InternVLChatModelWBboxML...')
        model = InternVLChatModelWBboxML(internvl_chat_config, vision_model, llm)
    ...
    for k, v in model.vision_model.encoder.named_parameters():
        if 'box_attentions' in k:
            v.requires_grad = True
