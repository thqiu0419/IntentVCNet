...
if __name__ == "__main__":
    ...
    for k, v in tqdm(annos.items()):
        for vid, vi in enumerate(v):
            ...
            for index, bbox in enumerate(normlized_bboxes):
                ...
                bboxes_str += f'{index}. ' + "<ref>" + f'{name}' + "</ref><box>[" + f'{bbox}' + "]</box>\n"
            
            # conversations item
            conversations = []
            q1 = {
                "from": "human",
                "value": f"<video>\nDescribe this video in one sentence, and focus on the {name} in the red coordinate box, whose coordinates at each frame are:\n{bboxes_str}."
                # "value": f"<video>\nDescribe this video in one sentence, and focus on {name} located in the coordinate list. The coordinates in each frame are:\n{bboxes_str}."
                # "value": f"<image>Describe this video and focus on {name} in the red box with coordinates {bboxes_str} in the video."
                }
            a1 = {
                "from": "gpt",
                "value": vi
    ...
    write_jsonl(final_data, file_path='shell/data/intentvc_data_f8_f32-redbox.jsonl')
    # # print(format_data)
